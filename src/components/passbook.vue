...
import { data, type User } from './db';
    ...
    onMounted(() => {
        ...
        papers.value = Array.from(document.querySelector(".book").querySelectorAll("[id^='p']") as NodeListOf<HTMLDivElement>) as HTMLDivElement[]
...
<template>
    ...
    <div id="book" ref="book" class="book relative hidden opacity-0">
        ...
        <div id="p1" class="paper">
            ...
            <div class="back">
                <div id="b1" class="back-content flex-col text-center gap-3 px-4 box-border py-5 ">
                    ...
                    <h2 class="text-2xl">Name: {{ currentUser.firstName }} {{ currentUser.lastName }} </h2>
                    <h2 class="text-2xl">Age: {{currentUser.age}}</h2>
                    <h2 class="text-2xl">Gender: {{ currentUser.gender }} </h2>
                    <div class="w-full h-1px border-b-1 border-b-solid border-b-black box-border px-2"></div>

                    <h2 class="text-2xl">Account Number: {{Math.floor(Math.random() * 10 ** 12)}} </h2>
                    <h2 class="text-2xl">Aadhar Number: {{currentUser.aadharNumber}} </h2>
                    <div class="w-full h-1px border-b-1 border-b-solid border-b-black box-border px-2"></div>
